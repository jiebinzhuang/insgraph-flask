...
from .exceptions import PageNotFound404, NoInstaPostPageFound
...
def extract_post_info(browser, postlink):
    ...
    try:
        logger.info("Scraping Post Link: " + postlink)
    ...
    try:
        ...
        logger.error("ERROR - getting Post infos (username) ")
    ...
    try:
        ...
        logger.info("location_id: " + str(location_id))
        logger.info("location_url: " + str(location_url))
        logger.info("location_name: " + str(location_name))
        logger.info("lat: " + str(lat))
        logger.info("lng: " + str(lng))
    except Exception as err:
        logger.warning("getting Location Infos (perhaps not set)")
    try:
        date = post.find_element_by_xpath('//a/time').get_attribute("datetime")
        logger.info("Post date: " + str(date))
    except:
        logger.error("ERROR - getting Post Date ")

    try:
        img_tags = post.find_elements_by_class_name('FFVAD')
        logger.info("number of images: " + str(len(img_tags)))
        for i in img_tags:
            imgs.append(i.get_attribute('src'))
            imgdesc.append(i.get_attribute('alt'))
            logger.info("post image: " + imgs[-1])
            logger.info("alt text: " + imgdesc[-1])
    except Exception as err:
        logger.error("ERROR - Post Image")
    ...
    try:
        ...
        if len(img_tags) >= 1:
            ...
            try:
                ...
                logger.info("video views: " + str(views))
            except:
                logger.error("ERROR - Getting Video Views")
    ...
    # except:
    #     logger.error("ERROR - getting Post comments function trying")
    ...
    try:
        ...
        logger.error("ERROR - getting Post caption/tags function")
...
def extract_post_caption(user_comments, username):
    ...
    try:
        ...
        logger.error("Error - getting caption")
        logger.error(err)
    return caption, tags
    ...
    try:
        if post.find_elements_by_tag_name('ul'):
            ...
            if len(comments) > 1:
                    ...
                    try:
                        ...
                        if (comments_found_last_run == len(comments)):
                            ...
                            if comments_run_same_length > 10:
                                logger.error("exit getting comments: " + str(
                ...
                logger.info("found comments: " + str(len(comments)))
            else:
            ...
            for comm in comments:
                try:
                    ...
                    logger.error("ERROR something went wrong getting user_commented")
                ...
                if (Settings.output_comments is True or len(user_comments) < 1):
                    ...
                    try:
                        ...
                        logger.error("ERROR something went wrong getting comment")

        logger.info(str(len(user_commented_list)) + " comments.")
...
def extract_post_likers(browser, post, postlink, likes):
    ...
    if (Settings.scrape_posts_likers is False):
        ...
        logger.info("GETTING LIKERS FROM POST")
    ...
    try:
        ...
        while len(likers_list) < likes:
            ...
            if tried_catch_likers > 10:
                logger.error("exit scrolling likers " + str(tried_catch_likers) + "x tries - liker list: " + str(
                    len(user_liked_list)) + " should be " + str(likes) + "")
                break
            likers_list_before = len(user_liked_list)

        logger.info('likers: ' + str(len(user_liked_list)))

    except BaseException as e:
        logger.error("Error - getting post likers")
        logger.error(e)
    return user_liked_list
