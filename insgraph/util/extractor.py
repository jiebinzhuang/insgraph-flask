...
from .extractor_posts import extract_post_info
...
def get_user_info(browser, username):
    ...
    logger.info("alias name: " + information['alias'])
...
def extract_followers(browser, username):
    logger.info('Extracting follower from ' + username)
    try:
...
def extract_information(browser, username, limit_amount):
    ...
    logger.info("User " + username + " has " + str(len(user_commented_total_list)) + " comments.")
